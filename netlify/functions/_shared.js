...
function navPairsFromContent(content, lang='tr'){ try{ const pages=lang==='en'?(content&&content.pages_en):(content&&content.pages); const raw=pages&&pages['__nav__']; if(raw&&String(raw).trim()){ const parsed=String(raw).split(/\r?\n/).map(l=>l.trim()).filter(Boolean).map(l=>{const i=l.lastIndexOf('|'); if(i<0) return null; const label=l.slice(0,i).trim(); const url=l.slice(i+1).trim(); return (label&&url)?[label,url]:null;}).filter(Boolean); if(parsed.length) return parsed; } }catch(e){} return lang==='en' ? [['Dictionary','/en/'],['Terms Index','/en/terms/'],['Publication Note','/en/publication-note/'],['Bibliography','/en/bibliography/'],['Privacy','/en/privacy-policy/'],['Cookies','/en/cookie-policy/'],['Terms of Use','/en/terms-of-use/'],['Contact','/en/contact/']] : [['Sözlük','/'],['Terimler Dizini','/terimler/'],['Yayın Notu','/yayin-notu/'],['Kaynakça','/kaynakca/'],['Gizlilik','/gizlilik-politikasi/'],['Çerezler','/cerez-politikasi/'],['Kullanım Şartları','/kullanim-sartlari/'],['İletişim','/iletisim/']]; }
function navHtml(lang='tr', content){ const items=navPairsFromContent(content,lang); return `<nav class="site-nav" aria-label="${lang==='en'?'Site links':'Site bağlantıları'}">`+items.map(i=>`<a href="${escapeAttr(i[1])}">${escapeHtml(i[0])}</a>`).join('')+'</nav>'; }
function footerLinksHtml(lang='tr', content){ const items=navPairsFromContent(content,lang); return items.map(i=>`<a href="${escapeAttr(i[1])}">${escapeHtml(i[0])}</a>`).join(' · '); }
...
function pageShell({ title, description, canonical, body, lang='tr', content }){ const s=siteName(lang); const home=lang==='en'?'/en/':'/'; const alt=altForPath(canonical,lang); return `<!doctype html><html lang="${lang}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>${escapeHtml(title)} · ${escapeHtml(s)}</title><meta name="description" content="${escapeAttr(description)}"><meta name="robots" content="index, follow"><link rel="canonical" href="${escapeAttr(canonical)}"><link rel="alternate" hreflang="tr" href="${escapeAttr(alt.tr)}"><link rel="alternate" hreflang="en" href="${escapeAttr(alt.en)}"><link rel="alternate" hreflang="x-default" href="${escapeAttr(alt.tr)}"><link rel="icon" href="/favicon.ico" sizes="any"><link rel="icon" type="image/png" href="/favicon-96.png" sizes="96x96"><link rel="apple-touch-icon" href="/apple-touch-icon.png"><style>${css()}</style></head><body><div class="site"><header class="topbar"><a class="brand" href="${home}" aria-label="${escapeAttr(s)}"><img class="brand__logo" src="/ats-logo.png" alt="ATS"><span class="brand__name">${escapeHtml(s)}</span></a>${langSwitch(lang)}</header>${navHtml(lang, content)}<main><h1>${escapeHtml(title)}</h1>${body}</main><footer class="site-footer"><p>© 2026 Ferdi Ertekin · ${escapeHtml(s)}</p><p>${footerLinksHtml(lang, content)}</p></footer></div></body></html>`; }
